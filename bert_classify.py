'''
@Author: gunjianpan
@Date:   2019-04-13 20:27:51
@Last Modified by:   gunjianpan
@Last Modified time: 2019-04-13 22:19:34
'''
...
bert_dir = '../../../../bert/uncased_L-24_H-1024_A-16/'
# Required parameters
flags.DEFINE_string(
    "data_dir", './',
...
flags.DEFINE_string("task_name", 'SemEval2017Task4',
...
class SemEval2017Task4Processor(DataProcessor):
    ...
    def load_data(self):
        ...
        train = [InputExample(guid='train-{}'.format(ii), text_a=tokenization.convert_to_unicode(
            jj), text_b=None, label=train_Y[ii]) for ii, jj in enumerate(train_X)]
        dev = [InputExample(guid='dev-{}'.format(ii), text_a=tokenization.convert_to_unicode(
            jj), text_b=None, label=y_test[ii]) for ii, jj in enumerate(X_test)]
        test = [InputExample(guid='test-{}'.format(ii), text_a=tokenization.convert_to_unicode(
...
def main(_):
    ...
    processors = {
        "cola": ColaProcessor,
        "semeval2017task4": SemEval2017Task4Processor,
        "mrpc": MrpcProcessor,
        "xnli": XnliProcessor,
    }
